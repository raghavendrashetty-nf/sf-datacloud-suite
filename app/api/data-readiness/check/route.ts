...
import { Catalog, CheckKey, CheckRequest, FieldType, SystemKey } from '@/lib/dataReadiness';
import { describeObjectFields, getConnectionInfo, runCheck } from '@/lib/salesforceClient';
import {
  describeZendeskObjectFields, getZendeskConnectionInfo, runZendeskCheck
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (system === 'salesforce') {
      ...
      return NextResponse.json({ error: 'Not connected to Salesforce.' }, { status: 400 });
    }

    // ---- Zendesk ----
    if (system === 'zendesk') {
      const info = getZendeskConnectionInfo();
      if (info.connected) {
        try {
          ...
          const result = await runZendeskCheck(checkType as CheckKey, object, field, fdef.mappedType as FieldType);
          return NextResponse.json({ result, mode: 'live' });
        } catch (e: any) {
          return NextResponse.json({ error: `Live check failed: ${e?.message ?? e}` }, { status: 500 });
        }
      }
      return NextResponse.json({ error: 'Not connected to Zendesk.' }, { status: 400 });
    }
