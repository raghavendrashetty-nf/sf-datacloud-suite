...
interface Props {
  item: RateItem; environment: Environment; itemPeriod: Period;
  volume: number; initial: number; overheadPct: number; costPerCreditUSD: number;
  result: ItemResult;
  onVolumeChange: (n: number) => void; onInitialChange: (n: number) => void; onPeriodChange: (p: Period) => void;
}
...
export default function ItemCard({ item, environment, itemPeriod, volume, initial, overheadPct, costPerCreditUSD, result, onVolumeChange, onInitialChange, onPeriodChange }: Props) {
    ...
    <div className={`card p-3 relative border-t-4 border-t-${theme.color}-500 ${isFree ? 'bg-emerald-50/30' : ''}`}>
      ...
      {isFree ? (
        <div className="mt-2 rounded-lg border border-emerald-200 bg-emerald-50 p-2 flex items-start gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5">
            <path d="M20 12l-8 8-4-4" /><circle cx="12" cy="12" r="10" />
          </svg>
          <p className="text-[11px] text-emerald-800 leading-snug">
            <strong>This item is free in {environment}.</strong> No credits are consumed no matter how many {item.unitLabel} you enter.
          </p>
        </div>
      ) : null}
      ...
      {item.supportsInitial ? (
        <div className={`mt-2 rounded-lg bg-${theme.color}-50 border border-${theme.color}-100 p-2`}>
          ...
          <p className="mt-1 text-[10px] italic text-slate-500 leading-snug">
            Credits consumed only during the initial Day&nbsp;0 load. Added once to the first-year total and never repeated.
          </p>
        </div>
      ) : null}
