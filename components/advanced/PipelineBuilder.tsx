...
function PipelineSection({ type, title, pipelines, rates, environment, overheadPct, onAdd, onUpdate, onRemove }: Props & { type: PipelineType; title: string }) {
  ...
  return (
    <div className={`card p-3 border-t-4 border-t-${accent}-500`}>
      ...
      {sectionPipelines.length === 0 ? (
        <p className="mt-3 text-xs text-slate-400 italic">No {type} pipelines configured yet.</p>
      ) : (
      ...
      <div className="p-2 bg-slate-50 space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          <button type="button" onClick={() => setExpanded((e) => !e)} aria-expanded={expanded}
            className="w-6 h-6 rounded-full bg-indigo-100 hover:bg-indigo-200 flex items-center justify-center shrink-0 transition-colors"
            aria-label={expanded ? 'Collapse pipeline' : 'Expand pipeline'} title={expanded ? 'Click to collapse' : 'Click to expand'}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
              className={`w-3.5 h-3.5 text-indigo-700 transition-transform ${expanded ? 'rotate-180' : ''}`}>
          ...
          {isOther ? (
            <input value={pipeline.customConnectionName ?? ''} onChange={(e) => onUpdate(pipeline.id, { customConnectionName: e.target.value })}
              placeholder="Specify connection name" className="w-40 border border-slate-200 rounded px-1.5 py-1 text-xs shrink-0" />
          ) : null}
          ...
          <div className="ml-auto text-right shrink-0">
            ...
            <div className="text-sm font-semibold tabular-nums text-slate-900">{fmtCredits(breakdown.annualCredits)}</div>
          </div>
          <button onClick={() => onRemove(pipeline.id)} className="text-rose-500 hover:text-rose-700 text-xs shrink-0" aria-label="Remove pipeline">Remove</button>
        </div>

        <div className="flex items-center gap-2 flex-wrap pl-8">
          <input value={pipeline.object} onChange={(e) => onUpdate(pipeline.id, { object: e.target.value })}
            placeholder="Object (e.g. Contact)" className="w-36 border border-slate-200 rounded px-1.5 py-1 text-xs shrink-0" />
            ...
            <>
              ...
              {pipeline.frequency === 'manual' ? (
                <input type="number" min={1} value={pipeline.manualRunsPerYear ?? 12}
                  ...
                  className="w-24 border border-slate-200 rounded px-1.5 py-1 text-xs" title="Runs per year" />
              ) : null}
            </>
          ) : (
      ...
      {expanded ? (
        <div className="p-3 border-t border-slate-200 bg-white space-y-3">
          ...
          <div className="text-[11px] text-slate-500 space-y-0.5">
            <div>Pipeline Name: <strong className="text-slate-700">{pipelineDisplayName(pipeline, connLabel)}</strong>
              <span className="text-slate-400"> ({pipeline.nameIsCustom ? 'custom' : 'auto-generated'} - click the name above to rename)</span>
            </div>
...
function EditablePipelineName({ pipeline, connLabel, onUpdate }: { pipeline: Pipeline; connLabel: string; onUpdate: Props['onUpdate'] }) {
  ...
  return (
    <button type="button" onClick={startEdit} title="Click to rename"
      className="min-w-0 flex-1 text-left font-semibold text-sm text-slate-900 hover:text-indigo-700 hover:underline truncate">
      {displayName}
    </button>
  );
}
...
function BreakdownStat({ label, credits, cost }: { label: string; credits: number; cost: number }) {
  return (
    <div className="rounded-lg border border-slate-200 p-2">
      ...
      <div className="text-sm font-semibold tabular-nums text-slate-900">{fmtCredits(credits)}</div>
