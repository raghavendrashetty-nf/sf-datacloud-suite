...
export interface SearchableItem { value: string; label: string; description?: string; badge?: string; }
interface Props {
  label: string;
  placeholder?: string;
  ...
  onQueryChange?: (query: string) => void; // NEW - fires as user types
  emptyMessage?: string;
  disabled?: boolean;
}
...
export default function SearchableSelect({
  ...
  items, value, onChange, onQueryChange,
  emptyMessage = 'No results', disabled = false
}: Props) {
  ...
  useEffect(() => {
    function onDoc(e: MouseEvent) { if (containerRef.current && !containerRef.current.contains(e.target as Node)) setOpen(false); }
    document.addEventListener('mousedown', onDoc);
    return () => document.removeEventListener('mousedown', onDoc);
  }, []);
  useEffect(() => { setHighlighted(0); }, [query]);
  useEffect(() => { if (!open) setQuery(selectedItem?.label ?? ''); }, [selectedItem, open]);

  // Notify parent whenever the search text changes
  ...
  function handleKey(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); const it = filtered[highlighted]; if (it) commit(it); }
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div className="relative">
        <input ref={inputRef} type="text" value={open ? query : (selectedItem?.label ?? '')}
        ...
        <ul role="listbox" className="absolute z-30 mt-1 w-full max-h-72 overflow-auto bg-white border border-slate-200 rounded-lg shadow-lg text-sm">
          {filtered.length === 0 ? (
            ...
            <li key={item.value} role="option" aria-selected={i === highlighted}
              ...
              className={`px-3 py-2 cursor-pointer flex items-start gap-2 ${i === highlighted ? 'bg-emerald-50' : ''}`}>
              <div className="flex-1 min-w-0">
                <div className="font-medium text-slate-900 truncate">{item.label}</div>
                {item.description ? <div className="text-[11px] text-slate-500 truncate">{item.description}</div> : null}
              </div>
              {item.badge ? <span className="chip bg-slate-100 text-slate-600 text-[10px] shrink-0 self-center">{item.badge}</span> : null}
            </li>
          ))}
        </ul>
      ) : null}
